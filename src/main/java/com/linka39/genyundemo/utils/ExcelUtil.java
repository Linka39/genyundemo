package com.linka39.genyundemo.utils;
...
/**
 * 路径：com.linka39.genyundemo.utils
 * 类名：
 * 功能：导入导出
 * 备注：
 * 创建人：typ
...
public class ExcelUtil {
    ...
    public static void main(String[] args) {
        try {
            String fileName = "f:/file.xls";
            List<Object[]> list = importExcel(fileName);
            for (int i = 0; i < list.size(); i++){
                User user = new User();
                user.setId(Integer.parseInt((String)list.get(i)[0]));
                user.setUsername((String) list.get(i)[1]);
                user.setPassword((String) list.get(i)[2]);
                user.setEnable(Integer.parseInt((String) list.get(i)[3]));
